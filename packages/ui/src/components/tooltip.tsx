    ...
    <TooltipPrimitive.Portal>
      ...
      >
        <TooltipPrimitive.Popup
          ...
          className={cn(
            "z-50 inline-flex w-fit max-w-xs origin-(--transform-origin) items-center gap-1.5 rounded-none bg-foreground px-3 py-1.5 text-xs uppercase tracking-wider text-background has-data-[slot=kbd]:pe-1.5 **:data-[slot=kbd]:relative **:data-[slot=kbd]:isolate **:data-[slot=kbd]:z-50 **:data-[slot=kbd]:rounded-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
            className
          )}
          {...props}
        >
          {children}
          <span className="absolute top-1.5 bottom-1.5 -left-1.5 h-1/2 w-1.5 bg-primary" aria-hidden="true" />
          <span className="absolute top-1.5 -right-1.5 bottom-1.5 h-1/2 w-1.5 bg-primary" aria-hidden="true" />
