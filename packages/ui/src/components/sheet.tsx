...
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        <div className="pointer-events-none absolute inset-x-0 top-0 h-1.5 bg-foreground dark:bg-ring" aria-hidden="true" />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1.5 bg-foreground dark:bg-ring" aria-hidden="true" />
        {showCloseButton && (
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col gap-1.5 p-6 uppercase tracking-[3px]", className)}
      {...props}
    />
  )
}
...
function SheetTitle({ className, ...props }: SheetPrimitive.Title.Props) {
  return (
    <SheetPrimitive.Title
      ...
      className={cn(
        "font-vector-battle text-base font-medium text-chart-3 [--text-shadow-color:var(--chart-3)] text-shadow-md",
        className
      )}
      {...props}
    />
